...
export function renderWorld(engine: GameEngine) {
  ...
  if(s===GameState.MENU || s===GameState.DIFFICULTY || s===GameState.DAILY_BRIEF || s===GameState.HEIST_INTRO) {
    ...
    if(intro&&!engine.settings.reduceMotion){
      ...
      const panX=-8*(1-smoothStep(.56,.84,p));
  ...
  if (p.hp > 0) {
    ...
    const interact=clamp((p.interactFlash??0)/12,0,1);
...
function renderHeistIntroUI(engine:GameEngine) {
  ...
  const mode=engine.pendingMode;
  const cfg=mode==='endless'
    ? {accent:'#d86b58',title:'ATRACO SIN FIN',meta:'RONDA 1 · SUPERVIVENCIA',status:'PRESIÓN ACTIVADA',kicker:'PROTOCOLO // SIN FIN'}
    : mode==='daily'
      ? {accent:'#79b9d2',title:'DESAFÍO DIARIO',meta:'OPERACIÓN DEL DÍA',status:'OPERACIÓN SINCRONIZADA',kicker:'PROTOCOLO // DIARIO'}
      : {accent:'#e6c56f',title:'EL BANCO DEL PAN',meta:'PISO 1 / 6 · '+difficultyLabel(engine),status:'INFILTRACIÓN AUTORIZADA',kicker:'ATRACO PRINCIPAL'};
  const cream='#f2e6be';

  ctx.save();

  // Letterbox muy breve: establece tono cinematográfico y desaparece antes
  // de que aparezca la información importante.
  const bars=Math.round(16*(1-smoothStep(.06,.40,p)));
  if(bars>0){
    ctx.fillStyle='rgba(3,9,12,.86)';
    ctx.fillRect(0,0,UI_BASE_WIDTH,bars);
    ctx.fillRect(0,CANVAS_HEIGHT-bars,UI_BASE_WIDTH,bars);
  }

  // Marca de entrada: rápida, no compite con la bóveda.
  const logoAlpha=(1-smoothStep(.08,.24,p))*smoothStep(0,.05,p);
  if(logoAlpha>0){
    ctx.globalAlpha=logoAlpha;
    drawTitleLogo(ctx,UI_BASE_WIDTH/2,59,engine.frame);
    ctx.globalAlpha=1;
  }

  // Briefing minimalista. Sin una tarjeta pesada: línea, título y estado.
  const infoIn=smoothStep(.43,.57,p);
  const infoOut=1-smoothStep(.84,.94,p);
  const infoAlpha=infoIn*infoOut;
  if(infoAlpha>0){
    const x=28,y=145,slide=13*(1-easeOutCubic(infoIn)),w=200;
    ctx.globalAlpha=infoAlpha;

    const shade=ctx.createLinearGradient(x-slide,y,x+w-slide,y);
    shade.addColorStop(0,'rgba(4,14,18,.78)');
    shade.addColorStop(.72,'rgba(4,14,18,.35)');
    shade.addColorStop(1,'rgba(4,14,18,0)');
    ctx.fillStyle=shade;ctx.fillRect(x-slide,y-18,w,99);

    ctx.fillStyle=cfg.accent;ctx.globalAlpha=infoAlpha*.88;ctx.fillRect(x-slide,y-18,3,99);
    ctx.fillRect(x+11-slide,y-8,62,1);
    ctx.globalAlpha=infoAlpha;

    text(ctx,cfg.kicker,x+12-slide,y,4.35,cfg.accent,'left',true,false);
    titleText(ctx,cfg.title,x+12-slide,y+31,12.5,cream,'left',false);
    text(ctx,cfg.meta,x+12-slide,y+51,4.85,'#91a4a3','left',true,false);

    const auth=smoothStep(.64,.75,p);
    ctx.globalAlpha=infoAlpha*auth;
    ctx.fillStyle='rgba(120,201,154,.085)';ctx.fillRect(x+12-slide,y+61,166,16);
    ctx.fillStyle=cfg.accent;ctx.fillRect(x+12-slide,y+61,3,16);
    text(ctx,cfg.status,x+23-slide,y+72,4.45,mode==='endless'?'#ef8b7a':mode==='daily'?'#9acde2':'#9bd8b4','left',true,false);
    ctx.globalAlpha=1;
  }

  // Telemetría superior pequeña, legible pero secundaria.
  const telemetry=smoothStep(.13,.28,p)*(1-smoothStep(.72,.88,p));
  if(telemetry>0){
    ctx.globalAlpha=telemetry;
    const label=p<.31?'CERROJOS // VERIFICANDO':p<.52?'BÓVEDA // DESBLOQUEANDO':'BÓVEDA // ABIERTA';
    text(ctx,label,24,27,4.15,'#71878b','left',true,false);
    const bw=118;
    ctx.fillStyle='rgba(255,255,255,.07)';ctx.fillRect(24,33,bw,2);
    ctx.fillStyle=cfg.accent;ctx.fillRect(24,33,bw*smoothStep(.10,.62,p),2);
    ctx.globalAlpha=1;
  }

  // Confirmación visual junto a la bóveda al final de la secuencia.
  const ready=smoothStep(.71,.79,p)*(1-smoothStep(.88,.95,p));
  if(ready>0){
    ctx.globalAlpha=ready;
    const rx=382,ry=112;
    ctx.fillStyle='rgba(4,15,18,.64)';ctx.fillRect(rx-48,ry-10,96,20);
    ctx.fillStyle=cfg.accent;ctx.fillRect(rx-48,ry-10,2,20);
    text(ctx,'ACCESO CONCEDIDO',rx,ry+3,4.15,mode==='endless'?'#ef8b7a':mode==='daily'?'#9acde2':'#a8dab9','center',true,false);
    ctx.globalAlpha=1;
  }

  if(elapsed>=HEIST_INTRO_SKIP_AFTER&&p<.88){
    const a=Math.min(1,(elapsed-HEIST_INTRO_SKIP_AFTER)/10)*(.62+.14*Math.sin(engine.frame*.08));
    ctx.globalAlpha=a;
    text(ctx,'ENTER / ESPACIO / CLIC · OMITIR',UI_BASE_WIDTH/2,CANVAS_HEIGHT-18,4.2,'#879997','center',true,false);
    ctx.globalAlpha=1;
  }

  // Pulso localizado al soltar los cerrojos.
  const flash=Math.sin(clamp((p-.255)/.14,0,1)*Math.PI);
  if(flash>0){
    const fg=ctx.createRadialGradient(336,170,20,336,170,125);
    fg.addColorStop(0,`rgba(255,231,163,${flash*.16})`);
    fg.addColorStop(1,'rgba(255,231,163,0)');
    ctx.fillStyle=fg;ctx.fillRect(205,42,275,260);
  }
...
function hudPlate(ctx:CanvasRenderingContext2D,x:number,y:number,w:number,h:number,accent='#e6c56f',alpha=.66){
  ctx.save();
  ctx.fillStyle=`rgba(4,12,16,${alpha})`;ctx.fillRect(x,y,w,h);
  ctx.strokeStyle='rgba(101,139,143,.24)';ctx.strokeRect(x+.5,y+.5,w-1,h-1);
  ctx.fillStyle=accent;ctx.globalAlpha=.55;ctx.fillRect(x,y,3,h);ctx.fillRect(x+3,y,w-3,1);ctx.globalAlpha=1;
  ctx.fillStyle='rgba(255,255,255,.025)';ctx.fillRect(x+6,y+4,w-12,1);
  ctx.restore();
}
...
function hudMeter(ctx:CanvasRenderingContext2D,x:number,y:number,w:number,value:number,accent:string){
  ...
  ctx.fillStyle='rgba(255,255,255,.24)';ctx.fillRect(x,y,Math.round(w*clamp(value,0,1)),1);
}
...
function drawHUD(engine: GameEngine) {
  ...
  const locW=engine.gameMode==='endless'?132:126,locX=CANVAS_WIDTH/2-locW/2;
  hudPlate(ctx,locX,4,locW,25,modeAccent,.58);
  if(engine.gameMode==='endless'){
    text(ctx,'ATRACO SIN FIN',CANVAS_WIDTH/2,13,5.3,modeAccent,'center',true,false);
    text(ctx,`R${Math.max(1,engine.endless.round)} · ${endlessStage(Math.max(1,engine.endless.round))}`,CANVAS_WIDTH/2,23,4.6,'#9bb0aa','center',true,false);
  }else{
  ...
  if(engine.alert>=5){
    const alert=Math.round(engine.alert),danger=alert>60;
    hudPlate(ctx,safeLeft+5,statusH+9,72,19,danger?'#dc6159':'#c99b66',.56);
    text(ctx,`ALERTA ${alert}`,safeLeft+12,statusH+20,4.8,danger?'#ff8f7f':'#d8b985','left',true,false);
    hudMeter(ctx,safeLeft+12,statusH+24,57,engine.alert/100,danger?'#e45b4f':'#c78868');
  }

  // Arsenal: dos ranuras compactas pero con jerarquía clara del arma activa.
  const slotW=94,slotH=27,baseX=safeLeft+5,baseY=CANVAS_HEIGHT-slotH-5;
  for(let i=0;i<2;i++){
    const weapon=p.weapons[i],active=p.activeWeapon===i,x=baseX+i*(slotW+5),y=baseY-(active?2:0);
    hudPlate(ctx,x,y,slotW,slotH,active?'#e6c56f':'#49626a',active?.74:.48);
    ctx.fillStyle=active?'#e6c56f':'#53686e';ctx.globalAlpha=active?.9:.42;ctx.fillRect(x,y,3,slotH);ctx.globalAlpha=1;

    const slide=active&&p.switchAnim>0?(1-p.switchAnim/12)*4-4:0;
    ctx.save();ctx.translate(x+8+slide,y+7);ctx.scale(active?1.05:.90,active?1.05:.90);ctx.globalAlpha=active?1:.48;
  ...
  // Objeto activo como tercer bloque del loadout, alineado a la derecha.
  if(p.activeItem){
    const x=safeRight-91,y=CANVAS_HEIGHT-32,ready=p.activeItemCooldown<=0,flash=p.quackReadyFlash>0;
    hudPlate(ctx,x,y,91,27,flash?'#f7dda0':ready?'#72c796':'#58696f',.60);
    ctx.save();ctx.translate(x+8,y+7);ctx.scale(ready?1.05:.92,ready?1.05:.92);ctx.globalAlpha=ready?1:.50;drawItem(ctx,0,0,p.activeItem,engine.frame);ctx.restore();
    const def=ACTIVE_ITEMS[p.activeItem];
    wrappedText(ctx,def?.name??'',x+28,y+11,56,5.25,6,2,ready?'#e8e0bd':'#7d8c90',true);
    if(ready) text(ctx,flash?'¡LISTO!':'LISTO',x+84,y+23,4.9,flash?'#f7dda0':'#72c796','right',true,false);
    else{
      const ratio=1-p.activeItemCooldown/p.activeItemMaxCooldown;
      hudMeter(ctx,x+29,y+22,46,ratio,'#e6c56f');
      text(ctx,`${(p.activeItemCooldown/(60*getBuild(p).cooldownRate)).toFixed(1)}s`,x+85,y+23,4.5,'#77888c','right',false,false);
    }
  }

  // Esquive centrado: icono funcional, no una tercera tarjeta.
  const dashReady=p.dashCooldown<=0,dashFlash=p.dashReadyFlash>0,dashW=54,dashX=CANVAS_WIDTH/2-dashW/2;
  hudMeter(ctx,dashX,CANVAS_HEIGHT-13,dashW,1-p.dashCooldown/(45*getBuild(p).dashCooldown),dashFlash?'#a3f0c2':dashReady?'#72c796':'#516168');
  ...
  if(engine.gameMode==='endless'){
    ...
    const pressureColor=e.pressure>=75?'#e55f55':e.pressure>=50?'#e6a04e':'#78b99a';
    hudPlate(ctx,safeLeft+6,56,116,42,pressureColor,.64);
    text(ctx,`RONDA ${Math.max(1,e.round)}`,safeLeft+14,68,6.2,'#f0d88d','left',true,false);
    text(ctx,`${e.threatRank} · ALERTA ${e.alert}`,safeLeft+14,79,4.6,'#9badab','left',false,false);
    hudMeter(ctx,safeLeft+14,84,96,e.pressure/100,pressureColor);
    const pressureState=e.pressure>=75?'CRÍTICO':e.pressure>=50?'PELIGRO':e.pressure>=25?'ALERTA':'CONTROL';
    text(ctx,`PRESIÓN ${Math.round(e.pressure)}% · ${pressureState}`,safeLeft+14,95,4.2,e.pressure>=75?'#ef8278':'#829698','left',e.pressure>=75,false);
    if(e.compositionLabel&&e.roundActive&&e.roundKind!=='boss'&&e.roundKind!=='subboss'&&e.roundKind!=='miniboss')text(ctx,e.compositionLabel,safeLeft+14,105,4.3,'#7f9992','left',true,false);
    if(e.milestone)text(ctx,e.milestone,CANVAS_WIDTH/2,36,5.8,e.round>=100?'#ff6c66':'#e6c56f','center',true,false);
  }
...
function drawBossBar(engine: GameEngine) {
  ...
  g.addColorStop(0,isFloorBoss?'#8d2930':isSubBoss?'#9a4c3b':'#7f672c');g.addColorStop(1,accent);
  ctx.fillStyle=g;ctx.fillRect(x+9,y+24,(w-18)*pct,5);
  ctx.fillStyle='rgba(255,255,255,.28)';ctx.fillRect(x+9,y+24,(w-18)*pct,1);

  for(let i=1;i<phaseCount;i++){
    const sx=x+9+((w-18)/phaseCount)*i;
    ctx.fillStyle='rgba(3,9,12,.96)';ctx.fillRect(sx,y+23,2,7);
  }

  // Módulos destructibles se resumen como nodos, sin añadir otra barra completa.
  if(boss.bossParts?.length){
    const live=boss.bossParts.filter(p=>!p.destroyed&&phase>=(p.exposedPhase??0));
    const exposed=boss.bossParts.filter(p=>phase>=(p.exposedPhase??0));
    const nodeX=x+w-10-exposed.length*5;
    exposed.forEach((part,i)=>{
      ctx.fillStyle=part.destroyed?'#4c3334':part.hp<part.maxHp?'#d79558':'#6fa38b';
      ctx.fillRect(nodeX+i*5,y+6,3,3);
    });
    if(exposed.length)text(ctx,`MÓDULOS ${live.length}/${exposed.length}`,x+w-10,y+18,3.7,'#718589','right',false,false);
  }
...
function renderConfirmUI(engine:GameEngine) {
  ...
  const body=kind==='restart'?'Se perderá el progreso no guardado de esta run y comenzarás de nuevo.':kind==='new_endless'?'El checkpoint actual será reemplazado cuando comience la nueva partida.':kind==='new_heist'?'La partida guardada del Atraco principal será reemplazada al comenzar la nueva operación.':kind==='quit'&&engine.gameMode==='heist'&&engine.heistCheckpointFloor>0?'Volverás al menú. Podrás continuar desde el inicio del piso guardado.':'Volverás al menú principal y la run actual terminará.';
  const detail=kind==='new_endless'&&engine.endlessCheckpointRound>0?'CHECKPOINT ACTUAL · RONDA '+engine.endlessCheckpointRound:kind==='new_heist'&&engine.heistCheckpointFloor>0?'CHECKPOINT ACTUAL · PISO '+engine.heistCheckpointFloor:kind==='quit'&&engine.gameMode==='heist'&&engine.heistCheckpointFloor>0?'GUARDADO · PISO '+engine.heistCheckpointFloor:'ESTA ACCIÓN NO SE PUEDE DESHACER';
