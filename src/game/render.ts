...
function drawDifficultyPips(ctx:CanvasRenderingContext2D,x:number,y:number,count:number,color:string) {
  for(let i=0;i<count;i++) {
    ctx.save();ctx.fillStyle=color;ctx.fillRect(x+i*11,y,6,2);ctx.fillRect(x+i*11+2,y-2,2,6);ctx.restore();
  }
}
...
function renderDifficultyUI(engine:GameEngine) {
  ...
  DIFFICULTY_MODES.forEach((mode,i)=>{
    ...
    const pipsX=tx+Math.min(126,def.label.length*5.8+8);drawDifficultyPips(ctx,pipsX,y+11,i+1,color);
