...
export const ITEM_ART: Record<string, IconPainter> = {
  ...
  quack_blaster: p => { /* pistola 9 mm */ p.rect(4,8,15,5,'#4d5963');p.rect(6,7,11,2,C.silver);p.rect(18,9,4,2,'#28323a');p.poly([[7,13],[13,13],[11,21],[6,20]],'#2b3138');p.rect(8,14,4,5,'#1a2026');p.rect(5,10,2,2,C.gold);p.dot(16,8,C.light); },
  breadcrumb_shotgun: p => { /* escopeta de corredera 12 ga */ p.line(2,18,13,11,'#5c3a28',4);p.line(9,12,21,5,'#3d4852',3);p.line(12,10,22,4,C.silver,2);p.rect(8,13,6,3,'#262e35');p.line(13,14,18,11,'#6a432c',3);p.rect(3,18,5,3,'#301f18'); },
  feather_gun: p => { /* subfusil 9 mm */ p.rect(3,9,16,6,'#303941');p.rect(5,8,10,2,'#65727b');p.rect(18,10,4,2,C.silver);p.rect(8,15,5,6,'#20272d');p.rect(12,14,4,7,'#39444c');p.line(3,12,1,16,'#22282d',2);p.rect(6,10,3,2,C.gold); },
  bread_boomerang: p => { /* carabina 5.56 */ p.rect(3,9,15,5,'#35414a');p.rect(6,8,9,2,'#71818b');p.rect(17,10,5,2,C.silver);p.poly([[4,14],[9,14],[6,21],[3,19]],'#262d33');p.rect(10,14,5,6,'#252d33');p.line(2,11,0,15,'#22282e',2);p.rect(8,6,6,2,'#1c252c');p.dot(10,6,C.gold); },
  rubber_duck_cannon: p => { /* fusil 7.62 */ p.rect(2,9,16,6,'#343a3e');p.rect(6,8,8,2,'#59656c');p.rect(17,10,5,2,'#8a979c');p.poly([[8,14],[13,14],[12,21],[7,20]],'#6c4327');p.line(3,13,1,18,'#6c4327',3);p.rect(13,14,5,3,'#6c4327');p.rect(7,10,3,2,C.gold); },
  baguette_launcher: p => { /* lanzagranadas 40 mm */ p.line(3,17,18,7,'#37454a',5);p.line(5,16,17,7,'#62757b',2);p.rect(8,15,5,6,'#202a2e');p.rect(17,5,5,5,'#2a3439');p.rect(18,6,4,3,'#8b965d');p.line(3,16,1,20,'#282f32',2);p.dot(20,6,C.gold); },
  quack_laser: p => { /* ametralladora ligera 5.56 */ p.rect(2,9,17,6,'#30383e');p.rect(5,8,9,2,'#68767e');p.rect(18,10,4,2,C.silver);p.poly([[6,14],[12,14],[11,21],[5,20]],'#282f34');p.rect(12,14,5,7,'#54616a');p.line(16,14,21,20,'#30383e',2);p.rect(7,6,8,2,'#222a30');p.dot(12,6,C.gold); },
  golden_egg_revolver: p => { /* revólver .357 */ p.rect(4,8,15,4,'#59656d');p.rect(17,9,5,2,C.silver);p.oval(10,12,4,4,'#7f8b91');p.oval(10,12,2,2,'#23292d');p.poly([[6,14],[11,14],[9,22],[4,20]],'#5d3b27');p.rect(8,15,2,4,'#2a1c14');p.dot(19,8,C.gold); },
  tactical_toaster: p => { /* pistola .45 suprimida */ p.rect(4,9,12,5,'#323b42');p.rect(15,10,7,2,'#1f272c');p.rect(17,9,5,4,'#252d32');p.poly([[7,14],[12,14],[10,21],[6,20]],'#252c31');p.rect(6,8,7,2,'#68747a');p.rect(5,11,2,2,'#6e826a'); },
  egg_cannon: p => { /* DMR 7.62 */ p.line(2,17,18,7,'#344047',4);p.line(5,15,20,6,'#69777e',2);p.rect(9,14,5,7,'#232b30');p.rect(11,5,8,3,'#1f272c');p.rect(14,4,4,2,C.silver);p.line(3,16,1,20,'#4f3527',2);p.rect(17,6,5,2,C.silver); },
  baguette_sniper: p => { /* fusil de cerrojo .308 */ p.line(2,17,20,6,'#3a454b',3);p.line(5,15,20,6,'#7c898e',2);p.rect(10,4,9,3,'#1d252a');p.rect(13,3,4,2,C.silver);p.rect(7,15,4,6,'#5d3b27');p.line(2,16,0,20,'#5d3b27',2);p.rect(18,5,5,2,C.silver); },
  plasma_baker: p => { /* rifle pesado .50 */ p.line(1,17,20,6,'#2d363b',5);p.line(5,14,22,5,'#69777d',2);p.rect(11,4,9,3,'#1c2327');p.rect(14,3,4,2,C.silver);p.rect(8,15,5,7,'#22292e');p.line(16,15,21,21,'#39444a',2);p.rect(20,4,3,3,'#22292e');p.dot(21,4,C.gold); },
  homing_crumbs: p => { /* PDW 5.7 */ p.rect(3,9,16,7,'#303b42');p.rect(17,11,5,2,C.silver);p.rect(7,14,8,5,'#252d32');p.poly([[4,10],[8,7],[15,7],[18,10]],'#4e5d65');p.rect(9,16,4,5,'#1f2529');p.rect(6,10,3,2,C.gold); },
