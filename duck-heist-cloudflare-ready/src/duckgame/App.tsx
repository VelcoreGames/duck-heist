...
import {
  createEngine,startGame,beginHeist,updateEngine,menuMove,buyUpgrade,saveSettings,getContentOf,chooseDifficulty,DIFFICULTY_ORDER,
  handleDash, handleActiveItem, cycleWeapon, confirmSwap, cancelSwap, confirmActiveSwap,
  selectSwapSlot,adjustSetting,setSettingFromPointer,toggleMute,SETTING_ROWS,wardrobeAction, ensureSkinVisible,selectEventOption,
...
import { MAIN_MENU,PAUSE_MENU,mainMenuHit,WARDROBE,WARDROBE_ACTION,wardrobeHit,swapHit,SETTINGS,SETTINGS_MUTE,inside,COLLECTION,activeSwapHit } from './game/layout';
...
import { cursorCss } from './menu/cursorPresets';
...
export default function App() {
  ...
  /** Escala responsiva uniforme: nunca deforma el pixel art ni la geometría del juego */
  ...
  useEffect(() => {
    ...
    engine.onStateChange=()=>{force(n=>n+1);setHint(hintFor(engine));setCursor(cursorCss(engine.settings.cursorStyle,engine.state===GameState.PLAYING));};
    ...
    applySize();
    setHint(hintFor(engine));setCursor(cursorCss(engine.settings.cursorStyle,false));
    ...
    const activateMenu = () => {
      ...
      switch (engine.menuIndex) {
        case 0: engine.difficultyIndex=Math.max(0,DIFFICULTY_ORDER.indexOf(engine.difficulty));goTo(GameState.DIFFICULTY);break;
    ...
    const onKeyDown = (e: KeyboardEvent,fromGamepad=false) => {
      ...
      if(k==='escape'&&document.fullscreenElement){
        ...
        if(now-lastFullscreenEscape<=600){lastFullscreenEscape=-Infinity;unlockFullscreenEscape();void document.exitFullscreen?.();return;}
        lastFullscreenEscape=now;
      } else if(k!=='escape') lastFullscreenEscape=-Infinity;
      ...
      switch (engine.state) {
        ...
        case GameState.DIFFICULTY:
          if(up)moveDifficulty(-1);else if(down)moveDifficulty(1);else if(yes)confirmDifficulty();else if(k==='escape'){playUiBack();goTo(GameState.MENU);}break;
    ...
    const onDown = (ev: MouseEvent) => {
      ...
      switch (engine.state) {
        ...
        case GameState.DIFFICULTY:{const i=hitList(x,y,DIFF_TOP,4,DIFF_H,DIFF_GAP,DIFF_W);if(i>=0){engine.difficultyIndex=i;confirmDifficulty();}else{playUiBack();goTo(GameState.MENU);}break;}
  ...
  return (
    <div className="duck-responsive-shell relative w-screen h-screen overflow-hidden bg-[#04050b] text-[#c3cbd9] flex flex-col items-center justify-center select-none"
      ...
      <div ref={wrapRef} className="duck-responsive-stage relative z-10"
        ...
        <canvas ref={uiRef} role="application" aria-label={translateText('Duck Heist. Usa WASD para moverte, flechas o clic izquierdo para disparar, clic derecho o Shift para esquivar, E para interactuar, M para el mapa y Escape para pausar.')} tabIndex={0} className="absolute inset-0 h-full w-full"
      ...
      <div className="duck-responsive-footer relative z-10 mt-6 flex max-w-[94vw] items-center gap-4 text-[10px] tracking-[0.12em] font-semibold uppercase">
        ...
        <button onClick={toggleFs}
          ...
          {translateText('Pantalla completa · F')}
        </button>
      </div>
...
type DuckKeyboardApi={lock?:(keys?:string[])=>Promise<void>;unlock?:()=>void};
function duckKeyboard():DuckKeyboardApi|undefined{return (navigator as Navigator&{keyboard?:DuckKeyboardApi}).keyboard;}
function lockFullscreenEscape(){try{const p=duckKeyboard()?.lock?.(['Escape']);if(p)void p.catch(()=>undefined);}catch{}}
function unlockFullscreenEscape(){try{duckKeyboard()?.unlock?.();}catch{}}

function toggleFullscreen(engine: GameEngine, after: () => void) {
  try {
    const el=document.documentElement;
    if(!document.fullscreenElement){
      void el.requestFullscreen?.().then(()=>{engine.settings.fullscreen=true;lockFullscreenEscape();saveSettings(engine);after();}).catch(()=>undefined);
    } else {
      unlockFullscreenEscape();
      void document.exitFullscreen?.().then(()=>{engine.settings.fullscreen=false;saveSettings(engine);after();}).catch(()=>undefined);
    }
  } catch {}
}

function hintFor(engine: GameEngine): string {
  if(engine.lastInput==='gamepad')return translateText("PALANCA IZQUIERDA · MOVER  RT · DISPARAR  B · ESQUIVAR  A · INTERACTUAR  Y · OBJETO  VIEW · MAPA  START · PAUSA");
  switch (engine.state) {
    case GameState.MENU:return translateText('W / S elegir · ENTER confirmar · rueda también vale');
    case GameState.DIFFICULTY:return translateText('W / S dificultad · ENTER confirmar · ESC volver');
    case GameState.PLAYING: return translateText('WASD mover · MOUSE / FLECHAS disparar · RUEDA cambiar arma · SHIFT esquivar · E interactuar · M mapa');
    case GameState.MAP:return translateText('MAPA · Combate en pausa · WASD / FLECHAS / MOUSE inspeccionar · M / ESC cerrar');
    case GameState.PAUSED: return translateText('ESC continuar · flechas navegar');
    default: return translateText('ENTER confirmar · ESC volver');
  }
}
