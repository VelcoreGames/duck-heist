...
export const DEFAULT_SETTINGS:Settings={master:.8,music:.35,sfx:.85,shake:.7,damageNumbers:true,uiScale:2,fullscreen:false,brightness:1,muted:false,cursorStyle:0};
...
export function normalizeProgress(raw:Record<string,unknown>={}) {
  ...
  settings.uiScale=2;settings.brightness=finite(values.brightness,1,.1,1);settings.shake=finite(values.shake,.7,0,1);settings.muted=values.muted===true;settings.cursorStyle=Math.floor(finite(values.cursorStyle,0,0,11));
