...
function duckTopologyList(rooms:any){return Array.isArray(rooms)?rooms:rooms&&typeof rooms.values==='function'?[...rooms.values()]:Object.values(rooms??{});}
function duckTopologyKey(x:number,y:number){return x+','+y;}
function duckTopologyNeighbors(room:any,list:any[]){const by=new Map(list.map((r:any)=>[duckTopologyKey(r.gx,r.gy),r])),dirs:any={N:[0,-1],S:[0,1],W:[-1,0],E:[1,0]},out:any[]=[];for(const d of room.doors??[]){const v=dirs[d];if(!v)continue;const n=by.get(duckTopologyKey(room.gx+v[0],room.gy+v[1]));if(n&&!out.includes(n))out.push(n);}return out;}
function duckTopologySpecial(r:any){return !!r&&[RoomType.ITEM,RoomType.SHOP,RoomType.EVENT,RoomType.TREASURE,RoomType.BOSS,RoomType.GUN_VAN].includes(r.type);}
function duckTopologyMovable(r:any){return !!r&&[RoomType.ITEM,RoomType.SHOP,RoomType.EVENT,RoomType.GUN_VAN].includes(r.type);}
function duckTopologyProtected(r:any){return !!r&&(r.type===RoomType.START||r.type===RoomType.BOSS||r.miniboss||r.isMiniboss||r.minibossType||r.bossType);}
function duckNormalizeSpecialTopology(rooms:any){const bag:any=rooms;if(!bag||bag.__duckTopologySafeV5)return;bag.__duckTopologySafeV5=true;const list:any[]=duckTopologyList(rooms),start=list.find((r:any)=>r?.type===RoomType.START);if(!start)return;const neighbors=(r:any)=>duckTopologyNeighbors(r,list),specialCount=(r:any,ignore:any=null)=>neighbors(r).filter((n:any)=>n!==ignore&&duckTopologySpecial(n)).length,directStart=(r:any)=>neighbors(start).includes(r);const hostSafe=(c:any,src:any)=>{if(!c||c===src||duckTopologyProtected(c)||duckTopologySpecial(c)||directStart(c))return false;const ns=neighbors(c);if(!ns.length)return false;for(const n of ns){if(n===src)continue;if(specialCount(n,src)>0)return false;}return true;};const move=(src:any)=>{if(!duckTopologyMovable(src))return false;const degree=(src.doors??[]).length;let pool=list.filter((c:any)=>hostSafe(c,src)&&(c.doors??[]).length===degree);if(!pool.length)pool=list.filter((c:any)=>hostSafe(c,src));if(!pool.length)return false;pool.sort((a:any,b:any)=>(b.distance??0)-(a.distance??0));const dst=pool[0],old=dst.type;dst.type=src.type;src.type=old;return true;};for(let pass=0;pass<8;pass++){let changed=false;for(const src of neighbors(start).filter(duckTopologyMovable))if(move(src))changed=true;for(const hub of list){if(duckTopologySpecial(hub)||duckTopologyProtected(hub))continue;const ss=neighbors(hub).filter(duckTopologySpecial);if(ss.length<=1)continue;const keep=ss.find((r:any)=>!duckTopologyMovable(r))??ss[0];for(const src of ss)if(src!==keep&&duckTopologyMovable(src)&&move(src))changed=true;}if(!changed)break;}}
...
export function generateMap(floorIndex: number,seed?:string): GameMap {
  ...
  if(remaining().length>6) if(random()<.38){assignDeadEndOrRandom(remaining(),RoomType.EVENT,random);}
...
export function generateRoomLayout(room: MapRoom,random=Math.random,forcedTemplate?:string): number[][] {
  ...
  duckNormalizeSpecialTopology(rooms);
  if (true) {
    return layout;
  }
