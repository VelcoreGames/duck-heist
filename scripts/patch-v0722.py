...
root=Path('.')
module_path=root/'src/game/graphics/chibiBossesV3.ts'
...
function birdBoss(c:Ctx,id:string,frame:number,ratio:number,goose:boolean=false,banker:boolean=false){
  ...
  const body=goose?CREAM:YELLOW,light=goose?CREAM_LIGHT:YELLOW_LIGHT,dark=goose?'#cabc9c':YELLOW_DARK;
...
function auditorBoss(c:Ctx,id:string,frame:number,ratio:number){
  if(id==='cajero_3000'){
...
render=render.replace('  drawProjectile, drawCoin, drawChest, drawBoss, drawDoor,','  drawProjectile, drawCoin, drawChest, drawDoor,',1)
marker="import { drawChibiFoodEnemyV3 } from './graphics/chibiFoodEnemiesV3';"
if marker not in render: raise SystemExit('food import missing')
render=render.replace(marker,marker+"\nimport { drawChibiBossV3 as drawBoss } from './graphics/chibiBossesV3';",1)
render=render.replace("  if (!usesChibiEnemyRenderer(e.type)) {","  if (!e.isBoss && !usesChibiEnemyRenderer(e.type)) {",1)
render=render.replace("  if (hurt && !usesChibiEnemyRenderer(e.type)) {","  if (hurt && !e.isBoss && !usesChibiEnemyRenderer(e.type)) {",1)
# unknown enemy fallback must no longer regress to old rectangular pigeon
render=render.replace("      default: drawSecurityPigeon(ctx, e.x, e.y, f, hurt);","      default: drawChibiBirdEnemyV3({ctx,x:e.x,y:e.y,size:e.size,frame:f,dirX,dirY,moving:Math.abs(e.vx)+Math.abs(e.vy)>.08,hurt,elite:e.elite,kind:'pigeon'});",1)
render=render.replace("        default:drawSecurityPigeon(ctx,-8,-8,f,false);","        default:drawChibiBirdEnemyV3({ctx,x:-d.enemy.size/2,y:-d.enemy.size/2,size:d.enemy.size,frame:f,dirX:1,dirY:0,moving:false,hurt:false,elite:d.enemy.elite,kind:'pigeon'});",1)
# old pigeon import is now unnecessary in this renderer
render=render.replace('  drawDuck, drawHeart, drawSecurityPigeon,','  drawDuck, drawHeart,',1)
render_path.write_text(render)

index=index_path.read_text()
if '0.7.21-tactical-chibi-consistency' not in index: raise SystemExit('v0.7.21 marker missing')
index=index.replace('0.7.21-tactical-chibi-consistency','0.7.22-bosses-chibi-pass')
index_path.write_text(index)
print('Applied v0.7.22 chibi bosses pass')
