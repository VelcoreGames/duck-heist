...
root = Path('.')
module_path = root / 'src/game/graphics/chibiEffectsV3.ts'
render_path = root / 'src/game/render.ts'
index_path = root / 'index.html'

module = r'''import type { Enemy, Particle, Projectile } from '../types';
...
if old_import not in render: raise SystemExit('sprite import block missing')
render = render.replace(old_import, new_import, 1)
anchor = "import { drawChibiFloorAtmosphereV3 } from './graphics/chibiAtmosphereV3';"
if anchor not in render: raise SystemExit('atmosphere import missing')
render = render.replace(anchor, anchor + "\nimport { drawChibiCoinV3, drawChibiProjectileV3, drawChibiParticleV3, drawChibiPickupGlowV3, drawChibiEnemyFxV3 } from './graphics/chibiEffectsV3';", 1)

render = render.replace("      // halo curativo\n      ctx.globalAlpha = 0.16;\n      ctx.fillStyle = '#ff8f9f';\n      ctx.beginPath();\n      ctx.arc(p.x, p.y, 12, 0, Math.PI * 2);\n      ctx.fill();\n      ctx.globalAlpha = 1;", "      drawChibiPickupGlowV3(ctx, p.x, p.y, f, '#ff9fa9', false);", 1)
render = render.replace("    } else drawCoin(ctx, p.x, p.y, f, p.type === 'golden_crumb');", "    } else drawChibiCoinV3(ctx, p.x, p.y, f, p.type === 'golden_crumb');", 1)
old_item_aura = "    ctx.globalAlpha=ITEMS[it.itemId]?.cursed?.3:.14;ctx.fillStyle=ITEMS[it.itemId]?.cursed?'#663174':RARITY_COLORS[def?.rarity ?? 3];ctx.beginPath();ctx.arc(it.x+8,fy+8,17,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;"
new_item_aura = "    drawChibiPickupGlowV3(ctx,it.x+8,fy+8,f,ITEMS[it.itemId]?.cursed?'#8c5a9d':RARITY_COLORS[def?.rarity ?? 3],true);"
if old_item_aura not in render: raise SystemExit('item aura missing')
render = render.replace(old_item_aura, new_item_aura, 1)

old_proj = """  for (const p of engine.projectiles) {
...
new_proj = """  for (const p of engine.projectiles) drawChibiProjectileV3(ctx,p,f);"""
if old_proj not in render: raise SystemExit('projectile loop missing')
render = render.replace(old_proj, new_proj, 1)
render = render.replace("  for (const pt of engine.particles) drawParticle(ctx, pt.x, pt.y, pt.type, pt.life, pt.color);", "  for (const pt of engine.particles) drawChibiParticleV3(ctx,pt,f);", 1)

pattern = r"  if \(e\.spawnAnim > 0\) \{.*?\n  // sombra legacy sólo para entidades que aún no usan renderer chibi propio"
replacement = """  const spawnAlpha = e.spawnAnim > 0 ? Math.max(0, 1 - e.spawnAnim / 18) : 1;
  drawChibiEnemyFxV3(ctx,e,f,'under');
  ctx.globalAlpha = spawnAlpha;

  // sombra legacy sólo para entidades que aún no usan renderer chibi propio"""
render, count = re.subn(pattern, replacement, render, count=1, flags=re.S)
if count != 1: raise SystemExit('enemy legacy fx block not found')
needle = """  if (!e.isBoss && e.hp < e.maxHp) {"""
if needle not in render: raise SystemExit('enemy hp block missing')
render = render.replace(needle, "  ctx.globalAlpha = 1;\n  drawChibiEnemyFxV3(ctx,e,f,'over');\n\n" + needle, 1)
render_path.write_text(render)

index = index_path.read_text()
if '0.7.27-themed-atmosphere-chibi' not in index: raise SystemExit('v0.7.27 marker missing')
index = index.replace('0.7.27-themed-atmosphere-chibi', '0.7.28-chibi-combat-fx')
index_path.write_text(index)
print('Applied v0.7.28 chibi combat effects pass')
