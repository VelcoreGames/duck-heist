...
root = Path('.')
module_path = root / 'src/game/graphics/chibiFloorArtV3.ts'
...
anchor = "import { drawChibiItemRoomDecorV3, drawChibiShopRoomDecorV3, drawChibiBossRoomDecorV3, drawChibiTreasureRoomDecorV3, drawChibiHiddenDoorV3 } from './graphics/chibiRoomDecorV3';"
if anchor not in render: raise SystemExit('room decor import missing')
render = render.replace(anchor, anchor + "\nimport { drawChibiFloorTileV3 } from './graphics/chibiFloorArtV3';", 1)
old = """      } else {
        drawRichTile(ctx, x, y, t === 1, special ? { floor: floorPal, wall: wallPal, trim: '#c58ae8', glow: '#c58ae8', deco: 'vault' } : th, room.gx, room.gy, f);
      }"""
new = """      } else if (!special && floorIndex > 0) {
        drawChibiFloorTileV3(ctx, x, y, t === 1, room.gx, room.gy, f, floorIndex);
      } else {
        drawRichTile(ctx, x, y, t === 1, special ? { floor: floorPal, wall: wallPal, trim: '#c58ae8', glow: '#c58ae8', deco: 'vault' } : th, room.gx, room.gy, f);
      }"""
if old not in render: raise SystemExit('tile branch missing')
render = render.replace(old, new, 1)
render_path.write_text(render)

index = index_path.read_text()
if '0.7.25-special-rooms-chibi' not in index: raise SystemExit('v0.7.25 marker missing')
index = index.replace('0.7.25-special-rooms-chibi', '0.7.26-themed-floors-chibi')
index_path.write_text(index)
print('Applied v0.7.26 themed floors chibi pass')
