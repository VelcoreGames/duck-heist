...
root = Path('.')
module_path = root / 'src/game/graphics/chibiCombatReadabilityV3.ts'
render_path = root / 'src/game/render.ts'
index_path = root / 'index.html'

module = r'''import type { Enemy } from '../types';
...
anchor = "import { drawChibiCoinV3, drawChibiProjectileV3, drawChibiParticleV3, drawChibiPickupGlowV3, drawChibiEnemyFxV3 } from './graphics/chibiEffectsV3';"
if anchor not in render: raise SystemExit('effects import missing')
render = render.replace(anchor, anchor + "\nimport { drawChibiEnemyHealthV3, drawChibiEnemyTelegraphV3 } from './graphics/chibiCombatReadabilityV3';", 1)

hp_pattern = r"  if \(!e\.isBoss && e\.hp < e\.maxHp\) \{.*?\n  \}\n\n  // --- AVISO DE ATAQUE \(telegrafía legible\) ---"
hp_repl = "  drawChibiEnemyHealthV3(ctx,e);\n\n  // --- AVISO DE ATAQUE (telegrafía chibi legible) ---"
render, count = re.subn(hp_pattern, hp_repl, render, count=1, flags=re.S)
if count != 1: raise SystemExit('enemy hp block not found')

tele_pattern = r"  if \(e\.telegraph > 0\.05\) \{.*?\n  \}\n\n  ctx\.globalAlpha = 1;"
tele_repl = """  if (e.telegraph > 0.05) {
    const cx = e.x + e.size / 2, cy = e.y + e.size / 2;
  ...
  ctx.globalAlpha = 1;"""
render, count = re.subn(tele_pattern, tele_repl, render, count=1, flags=re.S)
if count != 1: raise SystemExit('enemy telegraph block not found')
render_path.write_text(render)

index = index_path.read_text()
if '0.7.29-player-motion-v10' not in index: raise SystemExit('v0.7.29 marker missing')
index = index.replace('0.7.29-player-motion-v10', '0.7.30-combat-readability-chibi')
index_path.write_text(index)
print('Applied v0.7.30 combat readability pass')
