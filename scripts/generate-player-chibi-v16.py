...
def front_gun_layer(anchor: tuple[float,float], recoil: float=0.0, lowered: float=0.0, alpha: int=255):
...
def draw_duck(direction: str, state: str, i: int, n: int) -> Image.Image:
    ...
    if state!='celebrate':
        if direction in ('left','right'):
            ...
            anchor=(46.4,41.2+bob*.18)
            paste_front_gun(im,anchor,recoil,q['lowered'])
            ellipse(im,(39.0,37.5+bob*.16,47.2,46.6+bob*.16),WING,INK,1.1)
            translucent_ellipse(im,(40.1,38.6+bob*.16,44.3,40.0+bob*.16),'#f7db84',95)
        else:
            anchor=(49.8,31.5+bob)
