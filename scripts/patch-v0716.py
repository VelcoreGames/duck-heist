...
root = Path('.')
module_path = root / 'src/game/graphics/policeVariantsV3.ts'
render_path = root / 'src/game/render.ts'
index_path = root / 'index.html'

module_code = r'''import { drawChibiPoliceDuckV3 } from './enemyChibiV3';
...
import_marker = "import { drawChibiPoliceDuckV3 } from './graphics/enemyChibiV3';"
if import_marker not in render:
    raise SystemExit('base police import missing')
render = render.replace(import_marker, import_marker + "\nimport { drawChibiPoliceVariantV3 } from './graphics/policeVariantsV3';", 1)

rapid_old = "case 'policia_rapido': drawPoliciaRapido(ctx, e.x, e.y, f, hurt, dirX); break;"
rapid_new = """case 'policia_rapido':
        ...
        break;"""
if rapid_old not in render: raise SystemExit('rapid render marker missing')
render = render.replace(rapid_old, rapid_new, 1)

shot_old = "case 'policia_escopeta': drawPoliciaEscopeta(ctx, e.x, e.y, f, hurt, dirX, e.telegraph); break;"
shot_new = """case 'policia_escopeta':
        drawChibiPoliceVariantV3({
        ...
        break;"""
if shot_old not in render: raise SystemExit('shotgun render marker missing')
render = render.replace(shot_old, shot_new, 1)

riot_old = """case 'policia_antidisturbios':
        drawPoliciaAntidisturbios(ctx, e.x, e.y, f, hurt,
          { x: Math.cos(e.shieldAngle), y: Math.sin(e.shieldAngle) },
          e.chargeTimer > 0, e.recover > 0);
        break;"""
riot_new = """case 'policia_antidisturbios':
        drawChibiPoliceVariantV3({
        ...
        break;"""
if riot_old not in render: raise SystemExit('riot render marker missing')
render = render.replace(riot_old, riot_new, 1)
render_path.write_text(render)

index = index_path.read_text()
if '0.7.15-premium-hud' not in index:
    raise SystemExit('Expected v0.7.15 marker not found')
index = index.replace('0.7.15-premium-hud', '0.7.16-police-variants-chibi')
index_path.write_text(index)

print('Applied v0.7.16 police variants chibi pass')
