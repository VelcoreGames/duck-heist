...
root = Path('.')
module_path = root / 'src/game/graphics/chibiRoomDecorV3.ts'
...
export function drawChibiItemRoomDecorV3(ctx: Ctx, frame: number, floorIndex: number) {
  ...
  rr(ctx, cx - 78, cy - 56, 156, 112, 13, rug as unknown as string);
  ctx.restore();
...
'''
# CanvasGradient is valid fillStyle, but helper rr is string-typed. Replace that one rug call with direct path after generation.
module = module.replace("rr(ctx, cx - 78, cy - 56, 156, 112, 13, rug as unknown as string);", "ctx.fillStyle = rug; ctx.beginPath(); ctx.roundRect(cx - 78, cy - 56, 156, 112, 13); ctx.fill();")
module_path.write_text(module)

render = render_path.read_text()
anchor = "import { drawChibiChestV3, drawChibiPedestalV3, drawChibiCandleV3, drawChibiStairsV3 } from './graphics/chibiInteractablesV3';"
if anchor not in render: raise SystemExit('interactables import missing')
render = render.replace(anchor, anchor + "\nimport { drawChibiItemRoomDecorV3, drawChibiShopRoomDecorV3, drawChibiBossRoomDecorV3, drawChibiTreasureRoomDecorV3, drawChibiHiddenDoorV3 } from './graphics/chibiRoomDecorV3';", 1)
old_secret = """      const t=DOOR_TILE[d],x=t.x*32,y=t.y*32;
...
new_secret = """      const t=DOOR_TILE[d],x=t.x*32,y=t.y*32;
      drawChibiHiddenDoorV3(ctx,x,y,d,f,engine.map.floorIndex);continue;"""
if old_secret not in render: raise SystemExit('hidden secret door branch missing')
render = render.replace(old_secret, new_secret, 1)
start = render.index('  // brillos por tipo de sala\n')
end = render.index('\n}\n\nfunction drawPedestalFull', start)
new_decor = '''  // composición chibi por tipo de sala; solo arte, sin alterar colisiones.
  if (special) {
...
render = render[:start] + new_decor + render[end:]
render_path.write_text(render)

index = index_path.read_text()
if '0.7.24-chibi-interactables' not in index: raise SystemExit('v0.7.24 marker missing')
index = index.replace('0.7.24-chibi-interactables', '0.7.25-special-rooms-chibi')
index_path.write_text(index)
print('Applied v0.7.25 special rooms chibi pass')
